...
class TsoKpiaHelper{
    ...
    public function tso_kpi_process($date){

        $users = User::where('kpi_type_id',2)->get(); // get all tso      
        $base_line = $this->get_base_line(2);
        $weight    = $this->get_weight(2);

        foreach($users as $user){
  
            $user_ids = UserTerritory::where('user_territories.supervisor_id',$user->id)
                        ->distinct()
    ...
    private function manage_sales($target,$user_id,$date,$base_line,$weight,$user_kpi_code,$kpia,$kpia_sum){
        ...
        $actual = (float)$this->get_technician_tractor_sales_actual($user_kpi_code,$date) + ($kpia_sum->sp_tractor_actual ? $kpia_sum->sp_tractor_actual : 0);
        ...
        $actual =  (float)$this->get_technician_nm_pt_sales_actual($user_kpi_code,$date) + ($kpia_sum->sp_nmpt_actual ? $kpia_sum->sp_nmpt_actual : 0);
