...
@section('title','Pending Job Card List | Motors Service')
<style>
    label {
        font-size: 11px;
    }
                ...
                <div class="card-header">
                    @if(request()->get('from_date') && request()->get('to_date'))
                        <form action="{{ route('job.card.export') }}" class="float-right" method="post">
                            ...
                            <input type="hidden" name="chassis_number" value="{{ request()->get('chassis_number') }}">
                    ...
                    @endif
                    <form role="form" action="{{url('/pending_job_card')}}" method="get">
                        <div class="row">
                            <div class="col-md-2">
                        ...
                        <tbody>
                            @foreach ($job_cards as $job_card)
                                <?php
                                   ...
                                   // dd($job_card);
                                ?>
                             <tr style="background: {{ $job_card->color }}">
                              <td>{{$job_card->id}}</td>
                              <td class="text-center">
                                  @if($job_card->flag != 'hide')
