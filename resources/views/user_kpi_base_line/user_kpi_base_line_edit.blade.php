...
<div class="content-header">
    ...
    <div class="row mb-2">
        ...
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">UserKpiBaseLine Edit</li>
        </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <form class="" role="form" method="POST" action="{{ route('user_kpi_base_line.update',$user_kpi_base_line->id ) }}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                    ...
                    <div class="form-group{{ $errors->has('user_kpi_id') ? 'has-error' : '' }}">
                        ...
                        <select name="user_kpi_id" id="user_kpi_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus type="select"  required     >
                            ...
                            @foreach($user_kpis as $user_kpi)
                             <option value="{{$user_kpi->id}}"  @if($user_kpi->id == $user_kpi_base_line->user_kpi_id){{"selected"}} @endif >{{$user_kpi->name}}</option>
                        @endforeach
                        </select>
                    ...
                    <div class="form-group{{ $errors->has('kpi_group_id') ? 'has-error' : '' }}">
                        ...
                        <select name="kpi_group_id" id="kpi_group_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus type="select"  required     >
                            ...
                            @foreach($kpi_groups as $kpi_group)
                             <option value="{{$kpi_group->id}}"  @if($kpi_group->id == $user_kpi_base_line->kpi_group_id){{"selected"}} @endif >{{$kpi_group->name}}</option>
                        @endforeach
                        </select>
                ...
                <div class="col-sm-6">
                    <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                        ...
                        <input name="amount" type="number" id="amount" class="form-control"   value="{{$user_kpi_base_line->amount}}"    autofocus required  placeholder="Amount"     >
...
<script>document.title = 'UserKpiBaseLine | Edit';</script>
@endsection
