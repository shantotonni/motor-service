    ...
    <div class="container-fluid">
        ...
        <div class="col-12">
          ...
          <div class="card"> 
            ...
            <div class="card-body">
                <h4>Area WIse Job Card List  {{date('M-Y',strtotime(request()->get('date')))}}
                ...
                <tbody>
                    @foreach ($job_cards as $job_card)
                          ...
                          <td>{{$job_card->id}}</td>
                          <td>{{$job_card->technitian->username}}</td>
                          <td>{{date("d-m-Y",strtotime( $job_card->service_date))}}</td>
                          <td>@if($job_card->is_approved==1){{'Yes'}}@else{{"No"}}@endif</td>
                          <td>{{!empty($job_card->area) ? $job_card->area->name : ''}}</td>
                          <td>{{!empty($job_card->territory) ? $job_card->territory->name : ''}}</td>
                          <td>{{$job_card->engineer->name}}</td>
                          <td>{{$job_card->technitian->name}}</td>
                          <td>@if($job_card->participant_id){{$job_card->participant->name}}@endif</td>
                          <td>{{!empty($job_card->product) ? $job_card->product->name : ''}}</td>
                          <td>{{$job_card->call_type->name}}</td>
                          <td>{{!empty($job_card->service_type) ? $job_card->service_type->name : ''}}</td>
...
<script>document.title = "area_wise_job_cards";</script>
