...
<div class="content-header">
    ...
    <div class="row mb-2">
        ...
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">UserKpi Edit</li>
        </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <form class="" role="form" method="POST" action="{{ route('user_kpi.update',$user_kpi->id ) }}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                ...
                <div class="col-sm-6">
                    <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                        ...
                        <input name="date" type="text" id="date"class="form-control datepicker"  value="@if($user_kpi->date){{date("d-m-Y", strtotime($user_kpi->date))}}@endif"    autofocus required  placeholder="Date"  autocomplete="off"       >
                        @if ($errors->has('date'))
                    ...
                    <div class="form-group{{ $errors->has('user_id') ? 'has-error' : '' }}">
                        ...
                        <select name="user_id" id="user_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus type="select"  required     >
                            ...
                            @foreach($users as $user)
                             <option value="{{$user->id}}"  @if($user->id == $user_kpi->user_id){{"selected"}} @endif >{{$user->name}}</option>
                        @endforeach
                        </select>
                    ...
                    <div class="form-group{{ $errors->has('kpi_type_id') ? 'has-error' : '' }}">
                        ...
                        <select name="kpi_type_id" id="kpi_type_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus required     >
                            ...
                            @foreach($kpi_types as $kpi_type)
                             <option value="{{$kpi_type->id}}"  @if($kpi_type->id == $user_kpi->kpi_type_id){{"selected"}} @endif >{{$kpi_type->name}}</option>
                        @endforeach
                        </select>
                ...
                <div class="col-sm-6">
                    <div class="form-group {{ $errors->has('total_kpi_target') ? 'has-error' : '' }}">
                        ...
                        <input name="total_kpi_target" type="number" id="total_kpi_target" class="form-control"   value="{{$user_kpi->total_kpi_target}}"    autofocus step="any"  required  placeholder="Total kpi target"     >
                ...
                <div class="col-sm-6">
                    <div class="form-group {{ $errors->has('total_kpi_ach') ? 'has-error' : '' }}">
                        ...
                        <input name="total_kpi_ach" type="number" id="total_kpi_ach" class="form-control"   value="{{$user_kpi->total_kpi_ach}}"    autofocus step="any"  required  placeholder="Total kpi ach"     >
...
<script>document.title = 'UserKpi | Edit';</script>
@endsection
