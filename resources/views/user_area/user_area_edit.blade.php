...
<div class="content-header">
    ...
    <div class="row mb-2">
        ...
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">UserArea Edit</li>
        </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <form class="" role="form" method="POST" action="{{ route('user_area.update',$user_area->id ) }}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                    ...
                    <div class="form-group{{ $errors->has('area_id') ? 'has-error' : '' }}">
                        ...
                        <select name="area_id" id="area_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus type="select"  required     >
                            ...
                            @foreach($areas as $area)
                             <option value="{{$area->id}}"  @if($area->id == $user_area->area_id){{"selected"}} @endif >{{$area->name}}</option>
                        @endforeach
                        </select>
                    ...
                    <div class="form-group{{ $errors->has('user_id') ? 'has-error' : '' }}">
                        ...
                        <select name="user_id" id="user_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus type="select"  required     >
                            ...
                            @foreach($users as $user)
                             <option value="{{$user->id}}"  @if($user->id == $user_area->user_id){{"selected"}} @endif >{{$user->username}} - {{$user->name}}</option>
                        @endforeach
                        </select>
...
<script>document.title = 'UserArea | Edit';</script>
@endsection
