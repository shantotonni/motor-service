...
<div class="content-header">
    ...
    <div class="row mb-2">
        ...
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">JobCardDetail Edit</li>
        </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <form class="" role="form" method="POST" action="{{ route('job_card_detail.update',$job_card_detail->id ) }}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                    ...
                    <div class="form-group{{ $errors->has('job_card_id') ? 'has-error' : '' }}">
                        ...
                        <select name="job_card_id" id="job_card_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus type="select"  required     >
                            ...
                            @foreach($job_cards as $job_card)
                             <option value="{{$job_card->id}}"  @if($job_card->id == $job_card_detail->job_card_id){{"selected"}} @endif >{{$job_card->name}}</option>
                        @endforeach
                        </select>
                    ...
                    <div class="form-group{{ $errors->has('user_id') ? 'has-error' : '' }}">
                        ...
                        <select name="user_id" id="user_id" data-live-search="true" class="form-control select2" style="width: 100%;"  autofocus type="select"  required     >
                            ...
                            @foreach($users as $user)
                             <option value="{{$user->id}}"  @if($user->id == $job_card_detail->user_id){{"selected"}} @endif >{{$user->name}}</option>
                        @endforeach
                        </select>
...
<script>document.title = 'JobCardDetail | Edit';</script>
@endsection
